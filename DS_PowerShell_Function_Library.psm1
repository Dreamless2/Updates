...
#       -DS_CheckPathExists                   -> Checks if path exists
#       -DS_JoinPaths                         -> Join paths
# -Citrix functions
#     -Provisioning Server
...
Function DS_BindCertificateToIISPort {
    ...
    process {
        ...
        try {
            Get-Item iis:\sslbindings\* | Where-Object { $_.Port -eq $Port } | Remove-Item
        ...
        if ( Test-Path $Store ) {
            ...
            if ( Test-Path $CertFile ) {
                ...
                if ( $WindowsVersion -ge 10 ) {
                    # If the version equals 10 or higher this means that the OS is W10 / W2K16 or higher
                    # For a PFX file, a different PowerShell cmdlet is used than for a non-PFX file
                    if ( $CertFile.Substring($CertFile.length - 4, 4) -eq ".pfx" ) {
                        # Check if the current certificate file is a PFX file (this type of certificate contains a private key)
                    ...
                    # Run the following code when the OS is lower than W10/W2K16
                    try {
...
Function DS_JoinPaths {
    <#
        ...
        .EXAMPLE
        DS_JoinPaths -Paths "C:\Temp", "SubFolder", "File.txt"
    ...
    process {
        ...
        For ($i = 1; $i -lt $Paths.Length; $i++) {
            $CombinedPath = Join-Path -Path $CombinedPath -ChildPath $Paths[$i]
        }
...
Function DS_CheckPathExists {
    <#
        ...
        .EXAMPLE
        DS_CheckPathExists -Path "C:\Temp\MyFile.txt"
...
Function DS_CreateStoreFrontStore {
    ...
    process {
        ...
        if ( !($ExistingDeployments) ) {
            ...
            # One or more deployments exists
            $ExistingDeploymentFound = $False
            Foreach ( $Deployment in $ExistingDeployments ) {
                # Loop through each deployment and check if the URL matches the one defined in the variable $HostbaseUrl
                if ($Deployment.HostbaseUrl -eq $HostBaseUrl) {
                    # The deployment URL is the same as the one we want to add, so nothing to do
            ...
            If ( $ExistingDeploymentFound -eq $False ) {
                ...
                If ( [string]::IsNullOrEmpty($IISSiteDir) ) {
                    # In case no directory for the new IIS site was specified in the variable $IISSiteDir, a new, random one must be created
                    ...
                    $NewIISSiteNumber = ((((Get-ChildItem -Path IIS:\Sites).ID) | Measure-Object -Maximum).Maximum) + 1                                       # Retrieve all existing IIS sites ('Get-ChildItem -Path IIS:\Sites'); list only one property, namely the site ID (ID); than use the Measure-Object (measure -Maximum) to get the highest site ID and add 1 to determine the new site ID
        ...
        If ( [string]::IsNullOrEmpty($ReceiverVirtualPath) ) {
            # In case no directory for the Receiver for Web site was specified in the variable $ReceiverVirtualPath, a directory name will be automatically generated
