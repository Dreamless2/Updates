...
# -Windows functions
#     -Files and folders
#       -Acme_PurgeFiles                           -> Delete files in a directory
#       -Acme_CopyArchive                          -> Copy a file or multiple files
#       -Acme_NewDirectory                         -> Create a directory
#       -Acme_WipeDirectory                        -> Delete a directory
#       -Acme_MoveDirectory                        -> Move a directory
#       -Acme_DeleteFile                           -> Delete a file
#       -Acme_RenameItem                           -> Rename a file or registry value
#       -Acme_UnzipArchive                         -> Expand a zip file 
#       -Acme_AddStream                            -> Add content to a file
#       -Acme_GetStream                            -> Get content from file 
#       -Acme_MoveArchive                          -> Move a file or multiple files
#       -Acme_GetFileName                          -> Get a name of any file
#       -Acme_ISOSetupInstall                      -> Execute an EXE file located on a ISO image, with or without arguments.
#       -Acme_CopyWithProgress                     -> Copy files and folders showing a progress
#     -Installations and executables
#       -Acme_RunProcess                           -> Start a process
#       -Acme_RunProcessNoWait                     -> Start a process without wait 
#       -Acme_MSIInstall                           -> Install msi software
#       -Acme_SetupInstall                         -> Install exe software
#     -Logging
#       -Acme_Logging                              -> Write log file
#     -Registry
#       -Acme_NewRegKey                            -> Create a registry key
#       -Acme_DeleteRegKey                         -> Delete a registry key
#       -Acme_DeleteRegKeyValue                    -> Delete a registry value (any data type)
#       -Acme_ImportRegFile                        -> Import a registry (*.reg) file into the registry
#       -Acme_RenameRegKey                         -> Rename a registry key
#       -Acme_RenameRegKeyValue                    -> Rename a registry value
#       -Acme_SetRegKeyValue                       -> Create or modify a registry value (any data type)
#     -Services
#       -Acme_ModifyStartupService                 -> Change the startup type of a service (Boot, System, Automatic, Manual and Disabled)
#       -Acme_StopService                          -> Stop a service (including depend services)
#       -Acme_StartingService                      -> Start service (including depend services)
#     -System
#       -Acme_WipePrefetch                         -> Clear prefetch folder
#       -Acme_StartStopProcess                     -> Start and stop a process     
#       -Acme_ResolvePath                          -> Resolves a given path (Path or LiteralPath) and checks if it exists.
#       -Acme_ChangeLocation                       -> Change the current working directory
#       -Acme_GetLocation                          -> Get the current working directory
#       -Acme_CheckPath                            -> Check if a path exists
#       -Acme_InvokeCommand                        -> Invoke an external command
#
#     -Downloads
#       -Acme_ResolveUri                           -> Resolves a URI and retrieves information such as file size, last modified date, and filename.
#       -Acme_NovaDownloader                       -> Downloading a file with progress bar and some features
#
...
Function Acme_PurgeFiles {
    <#
        ...
        .EXAMPLE
        Acme_PurgeFiles -Directory "C:\Temp"
    ...
    begin {
        ...
        Acme_Logging "INFO" "START FUNCTION - $FunctionName" $LogFile
    }
 
    process {
        Acme_Logging "INFO" "Cleanup directory $Directory" $LogFile
        if (Test-Path $Directory ) {
            try {
                Remove-Item "$Directory\*.*" -Force -Recurse | Out-Null
                Remove-Item "$Directory\*" -Force -Recurse | Out-Null
                Acme_Logging "SUCCESS" "Successfully deleted all files and subfolders in the directory $Directory" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to delete files and subfolders in the directory $Directory (exit code: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
        else {
            Acme_Logging "INFO" "The directory $Directory does not exist. Nothing to do" $LogFile
        }
    }

    end {
        Acme_Logging "INFO" "END FUNCTION - $FunctionName" $LogFile
    }
}

#==========================================================================

# FUNCTION Acme_UnzipArchive
#==========================================================================
...
Function Acme_UnzipArchive {
    <#
        ...
        .EXAMPLE
        Acme_UnzipArchive -File "C:\Temp\MyArchive.zip" -DestinationPath "C:\Temp\ExtractedFiles" -Pattern $true
    ...
    process {
        ...
        Acme_Logging "INFO" "Source file: $File" $LogFile
        Acme_Logging "INFO" "Destination path: $DestinationPath" $LogFile

        # Check if the source file exists
        if (!(Test-Path $File)) {
            Acme_Logging "ERROR" "The file '$File' does not exist!" $LogFile
            Exit 1
        }

        # Check if the destination directory exists, if not, create it
        if (!(Test-Path $DestinationPath)) {
            Acme_Logging "INFO" "The destination directory does not exist. Creating directory: $DestinationPath" $LogFile
        ...
        try {
            if ($Pattern) {
                Acme_Logging "INFO" "Expanding archive '$File' to '$DestinationPath' with overwrite enabled" $LogFile
                Expand-Archive -Path $File -DestinationPath $DestinationPath -Force
            }
            else {
                Acme_Logging "INFO" "Expanding archive '$File' to '$DestinationPath' without overwriting" $LogFile
                Expand-Archive -Path $File -DestinationPath $DestinationPath
            }
            Acme_Logging "SUCCESS" "The archive '$File' was successfully extracted to '$DestinationPath'" $LogFile
        }
        catch {
            Acme_Logging "ERROR" "An error occurred while extracting the archive '$File' to '$DestinationPath': $_" $LogFile
            Exit 1
        }
    }
...
Function Acme_AddStream {
    <#
        ...
        .EXAMPLE
        Acme_AddStream -FilePath "C:\Logs\example.txt" -Content "This is a new line of text."
    ...
    process {
        ...
        Acme_Logging "INFO" "Adding content to the file '$FilePath'" $LogFile

        try {
            # Add the content to the specified file
            Add-Content -Path $FilePath -Value $Content
            Acme_Logging "INFO" "Content successfully added to '$FilePath'" $LogFile
        }
        catch {
            # Log any errors encountered during the content addition
            Acme_Logging "ERROR" "Error adding content to '$FilePath' (error: $($_.Exception.Message))" $LogFile
            Exit 1
        }
    }
...
Function Acme_GetStream {
    <#
        ...
        .EXAMPLE
        Acme_GetStream -FilePath "C:\Logs\example.txt"
    ...
    process {
        ...
        Acme_Logging "INFO" "Retrieving content from the file '$FilePath'" $LogFile
        ...
        try {
            ...
            if (Test-Path $FilePath) {
                ...
                Acme_Logging "INFO" "Content successfully retrieved from '$FilePath'" $LogFile
                return $Content
            }
            else {
                # Log a warning if the file does not exist
                Acme_Logging "WARNING" "File '$FilePath' does not exist." $LogFile
                return $null
            }
        }
        catch {
            # Log any errors encountered during the content retrieval
            Acme_Logging "ERROR" "Error retrieving content from '$FilePath' (error: $($_.Exception.Message))" $LogFile
            Exit 1
        }
    }
...
Function Acme_MoveArchive {
    <#
        ...
        .EXAMPLE
        Acme_MoveArchive -SourceFiles "C:\Temp\MyFile.txt" -Destination "C:\Temp2"
        Moves 'C:\Temp\MyFile.txt' to 'C:\Temp2'.
        .EXAMPLE
        Acme_MoveArchive -SourceFiles "C:\Temp\MyFile.txt" -Destination "C:\Temp2\MyRenamedFile.txt"
        Moves 'C:\Temp\MyFile.txt' to 'C:\Temp2' and renames it to 'MyRenamedFile.txt'.
        .EXAMPLE
        Acme_MoveArchive -SourceFiles "C:\Temp\*.txt" -Destination "C:\Temp2"
    ...
    process {
        ...
        Acme_Logging "INFO" "Move the source file(s) '$Files' to '$Destination'" $LogFile
        ...
        Acme_Logging "INFO" "Check if the destination path '$TempFolder' exists. If not, create it" $LogFile
        if (Test-Path $TempFolder) {
            Acme_Logging "INFO" "The destination path '$TempFolder' already exists. Nothing to do" $LogFile
        }
        else {
            Acme_Logging "INFO" "The destination path '$TempFolder' does not exist" $LogFile
            Acme_NewDirectory -Directory $TempFolder
        }

        # Move the source files
        Acme_Logging "INFO" "Start moving the source file(s) '$Files' to '$Destination'" $LogFile
        try {
            Move-Item -Path $Files -Destination $Destination -Force
            Acme_Logging "SUCCESS" "Successfully moved the source file(s) '$Files' to '$Destination'" $LogFile
        }
        catch {
            # Log the error if the move fails
            Acme_Logging "ERROR" "An error occurred while moving the source file(s) '$Files' to '$Destination'. Error: $($_.Exception.Message)" $LogFile
            Exit 1
        }
    }
...
Function Acme_CopyArchive {
    <#
        ...
        To see the examples, please enter the following PowerShell command: Get-Help Acme_CopyArchive -examples
        .PARAMETER Destination
        ...
        UNC paths can be used as well. The destination directory is automatically created if it does not exist (in this case the function 'Acme_NewDirectory' is called). 
        ...
        Please see the examples for more information.To see the examples, please enter the following PowerShell command: Get-Help Acme_CopyArchive -examples
        .EXAMPLE
        Acme_CopyArchive -Files "C:\Temp\MyFile.txt" -Destination "C:\Temp2"
        Copies the file 'C:\Temp\MyFile.txt' to the directory 'C:\Temp2'
        .EXAMPLE
        Acme_CopyArchive -Files "C:\Temp\MyFile.txt" -Destination "C:\Temp2\MyNewFileName.txt"
        Copies the file 'C:\Temp\MyFile.txt' to the directory 'C:\Temp2' and renames the file to 'MyNewFileName.txt'
        .EXAMPLE
        Acme_CopyArchive -Files "C:\Temp\*.txt" -Destination "C:\Temp2"
        Copies all files with the file extension '*.txt' in the directory 'C:\Temp' to the destination directory 'C:\Temp2'
        .EXAMPLE
        Acme_CopyArchive -Files "C:\Temp\*.*" -Destination "C:\Temp2"
        ...
        .EXAMPLE
        Acme_CopyArchive -Files "C:\Temp\*.txt" -Destination "\\localhost\Temp2"
    ...
    process {
        Acme_Logging "INFO" "Copy the source file(s) '$Files' to '$Destination'" $LogFile
        ...
        Acme_Logging "INFO" "Start copying the source file(s) '$Files' to '$Destination'" $LogFile
        try {
            Copy-Item $Files -Destination $Destination -Force -Recurse
            Acme_Logging "SUCCESS" "Successfully copied the source files(s) '$Files' to '$Destination'" $LogFile
        }
        catch {
            Acme_Logging "ERROR" "An error occurred trying to copy the source files(s) '$Files' to '$Destination'" $LogFile
            Exit 1
        }
    }
...
Function Acme_NewDirectory {
    <#
        ...
        .EXAMPLE
        Acme_NewDirectory -Directory "C:\Temp\MyNewFolder"
    ...
    process {
        Acme_Logging "INFO" "Create directory $Directory" $LogFile
        if ( Test-Path $Directory ) {
            Acme_Logging "INFO" "The directory $Directory already exists. Nothing to do" $LogFile
        }
        else {
            try {
                New-Item -ItemType Directory -Path $Directory -Force | Out-Null
                Acme_Logging "SUCCESS" "Successfully created the directory $Directory" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to create the directory $Directory (exit code: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
    }
...
Function Acme_WipeDirectory {
    <#
        ...
        .EXAMPLE
        Acme_WipeDirectory -Directory "C:\Temp\MyFolder"
    ...
    process {
        Acme_Logging "INFO" "Delete directory $Directory" $LogFile
        if ( Test-Path $Directory ) {
            try {
                Remove-Item $Directory -Force -Recurse | Out-Null
                Acme_Logging "SUCCESS" "Successfully deleted the directory $Directory" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to delete the directory $Directory (exit code: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
        else {
...
Function Acme_DeleteFile {
    <#
        ...
        .EXAMPLE
        Acme_DeleteFile -File "C:\Temp\MyFile.txt"
        Deletes the file "C:\Temp\MyFile.txt"
        .EXAMPLE
        Acme_DeleteFile -File "C:\Temp\*.txt"
        ...
        .EXAMPLE
        Acme_DeleteFile -File "C:\Temp\*.*"
        Deletes all files in the directory "C:\Temp". This function does NOT remove any subfolders nor files within a subfolder (use the function 'Acme_PurgeFiles' instead)
    #>
    [CmdletBinding()]
    Param( 
        [Alias("F")]
    ...
    process {
        Acme_Logging "INFO" "Delete the file '$File'" $LogFile
        if ( Test-Path $File ) {
            try {
                Remove-Item "$File" | Out-Null
                Acme_Logging "SUCCESS" "Successfully deleted the file '$File'" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to delete the file '$File' (exit code: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
        else {
            Acme_Logging "INFO" "The file '$File' does not exist. Nothing to do" $LogFile
        }
    }
...
Function Acme_MoveDirectory {
    <#
        ...
        .EXAMPLE
        Acme_MoveDirectory -SourceDirectories "C:\Temp\MyFolder" -Destination "C:\Temp2"
        Moves 'C:\Temp\MyFolder' to 'C:\Temp2'.
        .EXAMPLE
        Acme_MoveDirectory -SourceDirectories "C:\Temp\MyFolder" -Destination "C:\Temp2\MyRenamedFolder"
        Moves 'C:\Temp\MyFolder' to 'C:\Temp2' and renames it to 'MyRenamedFolder'.
        .EXAMPLE
        Acme_MoveDirectory -SourceDirectories "C:\Temp\*" -Destination "C:\Temp2"
    ...
    process {
        # Log the source and destination details
        Acme_Logging "INFO" "Move the source directory(s) '$SourceDirectories' to '$Destination'" $LogFile
        ...
        Acme_Logging "INFO" "Start moving the source directory(s) '$SourceDirectories' to '$Destination'" $LogFile
        try {
            Move-Item -Path $SourceDirectories -Destination $Destination -Force
            Acme_Logging "SUCCESS" "Successfully moved the source directory(s) '$SourceDirectories' to '$Destination'" $LogFile
        }
        catch {
            # Log the error if the move fails
            Acme_Logging "ERROR" "An error occurred while moving the source directory(s) '$SourceDirectories' to '$Destination'. Error: $($_.Exception.Message)" $LogFile
            Exit 1
        }
    }
...
Function Acme_RenameItem {
    <#
        ...
        .EXAMPLE
        Acme_RenameItem -ItemPath "C:\Temp\MyOldFileName.txt" -NewName "MyNewFileName.txt"
        ...
        .EXAMPLE
        Acme_RenameItem -ItemPath "C:\Temp\MyOldFileName.txt" -NewName "MyNewFileName.rtf"
        ...
        .EXAMPLE
        Acme_RenameItem -ItemPath "C:\Temp\MyOldFolderName" -NewName "MyNewFolderName"
    ...
    process {
        Acme_Logging "INFO" "Rename '$ItemPath' to '$NewName'" $LogFile

        # Rename the item (if exist)
        if ( Test-Path $ItemPath ) {
            try {
                Rename-Item -path $ItemPath -NewName $NewName | Out-Null
                Acme_Logging "SUCCESS" "The item '$ItemPath' was renamed to '$NewName' successfully" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to rename the item '$ItemPath' to '$NewName' (exit code: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
        else {
            Acme_Logging "INFO" "The item '$ItemPath' does not exist. Nothing to do" $LogFile
        }
    }
...
Function Acme_RunProcess {
    ...
    process {
        if ($null -eq $Arguments -or $Arguments.Count -eq 0) {
            Acme_Logging "INFO" "Execute process '$FileName' with no arguments" $LogFile

            if (-not (Test-Path $FileName)) {
                Acme_Logging "ERROR" "Executable not found: '$FileName'" $LogFile
                Exit 1
            }
            ...
            Acme_Logging "INFO" "Execute process '$FileName' with $($Arguments.Count) arguments" $LogFile
        ...
        if ($ProcessExitCode -eq 0) {
            Acme_Logging "SUCCESS" "The process '$FileName' ended successfully" $LogFile
        }
        else {
            Acme_Logging "ERROR" "An error occurred trying to execute the process '$FileName' (exit code: $ProcessExitCode)!" $LogFile
...
# FUNCTION Acme_RunProcessNoWait
#==========================================================================
Function Acme_RunProcessNoWait {
    <#
        .SYNOPSIS
        ...
        .EXAMPLE
        Acme_RunProcessNoWait -FileName "C:\Temp\MyApp.exe" -Arguments "-silent"
    ...
    process {
        try {
            if ([string]::IsNullOrEmpty($Arguments)) {
                Acme_Logging "INFO" "Execute process '$FileName' with no arguments without wait." $LogFile
                ...
                Acme_Logging "INFO" "Execute process '$FileName' with arguments '$Arguments' without wait." $LogFile
            ...
            Acme_Logging "SUCCESS" "The process '$FileName' was started successfully." $LogFile
        }
        catch {
            Acme_Logging "ERROR" "Failed to start process '$FileName'. Exception: $_" $LogFile
            Exit 1
        }
    }
...
Function Acme_NewRegKey {
    <#
        ...
        .EXAMPLE
        Acme_NewRegKey -RegKeyPath "hklm:\Software\MyApp"
    ...
    process {
        Acme_Logging "INFO" "Create registry key '$RegKeyPath'" $LogFile
        if ( Test-Path $RegKeyPath ) {
            Acme_Logging "INFO" "The registry key '$RegKeyPath' already exists. Nothing to do" $LogFile
        }
        else {
            try {
                New-Item -Path $RegkeyPath -Force | Out-Null
                Acme_Logging "SUCCESS" "The registry key '$RegKeyPath' was created successfully" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to create the registry key '$RegKeyPath' (exit code: $($Error[0]))!" $LogFile
                Acme_Logging "INFO" "Note: define the registry path as follows: hklm:\Software\MyApp" $LogFile
                Exit 1
            }
        }
    }
...
# FUNCTION Acme_DeleteRegKeyValue
#==========================================================================
Function Acme_DeleteRegKeyValue {
    <#
        .SYNOPSIS
        ...
        .EXAMPLE
        Acme_DeleteRegKeyValue -RegKeyPath "hklm:\SOFTWARE\MyApp" -RegValueName "MyValue"
    ...
    process {
        Acme_Logging "INFO" "Delete registry value '$RegValueName' in '$RegKeyPath'" $LogFile
        ...
        try {
            ...
            Acme_Logging "INFO" "The registry value '$RegValueName' in the registry key '$RegKeyPath' does NOT exist. Nothing to do" $LogFile
        }
        ...
        if ( $RegValueExists -eq $True ) {
            try {
                ...
                Acme_Logging "SUCCESS" "The registry value '$RegValueName' in the registry key '$RegKeyPath' was deleted successfully" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to delete the registry value '$RegValueName' in the registry key '$RegKeyPath' to '$NewName' (exit code: $($Error[0]))!" $LogFile
                Acme_Logging "INFO" "Note: define the registry path as follows: hklm:\SOFTWARE\MyApp" $LogFile
                Exit 1
            }
        }
...
Function Acme_ImportRegFile {
    <#
        ...
        .EXAMPLE
        Acme_ImportRegFile-FileName "C:\Temp\MyRegFile.reg"
    ...
    process {
        Acme_Logging "INFO" "Import registry file '$FileName'" $LogFile
        if ( Test-Path $FileName ) {
            try {
                $process = start-process -FilePath "reg.exe" -ArgumentList "IMPORT ""$FileName""" -WindowStyle Hidden -Wait -PassThru
                if ( $process.ExitCode -eq 0 ) {
                    Acme_Logging "SUCCESS" "The registry settings were imported successfully (exit code: $($process.ExitCode))" $LogFile
                }
                else {
                    Acme_Logging "ERROR" "An error occurred trying to import registry settings (exit code: $($process.ExitCode))" $LogFile				
                    Exit 1
                }
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to import the registry file '$FileName' (exit code: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
        else {
            Acme_Logging "ERROR" "The file '$FileName' does NOT exist!" $LogFile
            Exit 1
        }
    }
...
# FUNCTION Acme_RenameRegKey
#==========================================================================
Function Acme_RenameRegKey {
    <#
        .SYNOPSIS
        Rename a registry key (for registry values use the function 'Acme_RenameRegKeyValue' instead)
        .DESCRIPTION
        Rename a registry key (for registry values use the function 'Acme_RenameRegKeyValue' instead)
        .PARAMETER RegKeyPath
        ...
        .EXAMPLE
        Acme_RenameRegKey -RegKeyPath "hklm:\Software\MyRegKey" -NewName "MyRegKeyNew"
    ...
    process {
        Acme_Logging "INFO" "Rename '$RegKeyPath' to '$NewName'" $LogFile

        # Rename the registry path (if exist)
        if ( Test-Path $RegKeyPath ) {
            try {
                Rename-Item -path $RegKeyPath -NewName $NewName | Out-Null
                Acme_Logging "SUCCESS" "The registry path '$RegKeyPath' was renamed to '$NewName' successfully" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to rename the registry path '$RegKeyPath' to '$NewName' (exit code: $($Error[0]))!" $LogFile
                Acme_Logging "INFO" "Note: define the registry path as follows: hklm:\SOFTWARE\MyApp" $LogFile
                Exit 1
            }
        }
        else {
            Acme_Logging "INFO" "The registry path '$RegKeyPath' does not exist. Nothing to do" $LogFile
        }
    }
...
# FUNCTION Acme_RenameRegKeyValue
# Note: this function works for registry values only. To rename a registry key, use the function 'Acme_RenameRegKey'.
#==========================================================================
Function Acme_RenameRegKeyValue {
    <#
        .SYNOPSIS
        Rename a registry value (all data types). To rename a registry key, use the function 'Acme_RenameRegKey'
        .DESCRIPTION
        Rename a registry value (all data types). To rename a registry key, use the function 'Acme_RenameRegKey'
        .PARAMETER RegKeyPath
        ...
        .EXAMPLE
        Acme_RenameRegKeyValue -RegKeyPath "hklm:\Software\MyRegKey" -RegValueName "MyRegistryValue" -NewName "MyRegistryValueNewName"
    ...
    process {
        Acme_Logging "INFO" "Rename the registry value '$RegValueName' in the registry key '$RegKeyPath' to '$NewName'" $LogFile
        ...
        if ( $RegValueExists -eq $True ) {
            try {
                ...
                Acme_Logging "SUCCESS" "The registry value '$RegValueName' in the registry key '$RegKeyPath' was successfully renamed to '$NewName'" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to rename the registry value '$RegValueName' in the registry key '$RegKeyPath' to '$NewName' (exit code: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
    }
...
Function Acme_SetRegKeyValue {
    <#
        ...
        .EXAMPLE
        Acme_SetRegKeyValue -RegKeyPath "hklm:\Software\MyApp" -RegValueName "MyStringValue" -RegValue "Enabled" -Type "String"
        Creates a new string value called 'MyStringValue' with the value of 'Enabled'
        .Example
        Acme_SetRegKeyValue -RegKeyPath "hklm:\Software\MyApp" -RegValueName "MyBinaryValue" -RegValue "01" -Type "Binary"
        Creates a new binary value called 'MyBinaryValue' with the value of '01'
        .Example
        Acme_SetRegKeyValue -RegKeyPath "hklm:\Software\MyApp" -RegValueName "MyDWORDValue" -RegValue "1" -Type "DWORD"
        Creates a new DWORD value called 'MyDWORDValue' with the value of 00000001 (or simply 1)
        .Example
        Acme_SetRegKeyValue -RegKeyPath "hklm:\Software\MyApp" -RegValueName "MyQWORDValue" -RegValue "1" -Type "QWORD"
        Creates a new QWORD value called 'MyQWORDValue' with the value of 1
        .Example
        Acme_SetRegKeyValue -RegKeyPath "hklm:\Software\MyApp" -RegValueName "MyMultiStringValue" -RegValue "Value1","Value2","Value3" -Type "MultiString"
        Creates a new multistring value called 'MyMultiStringValue' with the value of 'Value1 Value2 Value3'
        .Example
        Acme_SetRegKeyValue -RegKeyPath "hklm:\Software\MyApp" -RegValueName "MyExpandStringValue" -RegValue "MyValue" -Type "ExpandString"
    ...
    process {
        Acme_Logging "INFO" "Set registry value $RegValueName = $RegValue (type $Type) in $RegKeyPath" $LogFile

        # Create the registry key in case it does not exist
        if ( !( Test-Path $RegKeyPath ) ) {
            Acme_NewRegKey $RegKeyPath
        }
        ...
        try {
            ...
            Acme_Logging "SUCCESS" "The registry value $RegValueName = $RegValue (type $Type) in $RegKeyPath was set successfully" $LogFile
        }
        catch {
            Acme_Logging "ERROR" "An error occurred trying to set the registry value $RegValueName = $RegValue (type $Type) in $RegKeyPath" $LogFile
            Acme_Logging "INFO" "Note: define the registry path as follows: hklm:\Software\MyApp" $LogFile
            Exit 1
        }
    }
...
# FUNCTION Acme_ModifyStartupService
...
Function Acme_ModifyStartupService {
    <#
        ...
        .EXAMPLE
        Acme_ModifyStartupService -ServiceName "Spooler" -StartupType "Disabled"
        Disables the service 'Spooler' (display name: 'Print Spooler')
        .EXAMPLE
        Acme_ModifyStartupService -ServiceName "Spooler" -StartupType "Manual"
    ...
    process {
        Acme_Logging "INFO" "Change the startup type of the service '$ServiceName' to '$StartupType'" $LogFile
        ...
        If ( Get-Service $ServiceName -erroraction silentlycontinue) {
            ...
            try {
                ...
                Acme_Logging "INFO" "The startup type of the service '$ServiceName' was successfully changed to '$StartupType'" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "An error occurred trying to change the startup type of the service '$ServiceName' to '$StartupType' (error: $($Error[0]))!" $LogFile
                Exit 1
            }
        }
        else {
            Acme_Logging "INFO" "The service '$ServiceName' does not exist. Nothing to do" $LogFile
        }
    }
...
Function Acme_StopService {
    <#
        ...
        .EXAMPLE
        Acme_StopService -ServiceName "Spooler"
    ...
    process {
        Acme_Logging "INFO" "Stop service '$ServiceName' ..." $LogFile
        ...
        If ( Get-Service $ServiceName -erroraction silentlycontinue) {
            ...
            If ( ((Get-Service $ServiceName -ErrorAction SilentlyContinue).Status) -eq "Running" ) {
                ...
                Acme_Logging "INFO" "Check for depend services for service '$ServiceName' and stop them" $LogFile
                ...
                If ($DependServices.Count -gt 0 ) {
                    foreach ($Service in $DependServices) {
                        Acme_Logging "INFO" "Depend service found: $Service" $LogFile
                        Acme_StopService -ServiceName $Service
                    }
                }
                else {
                    Acme_Logging "INFO" "No depend service found" $LogFile
                }
                ...
                try {
                    ...
                    Acme_Logging "ERROR" "An error occurred trying to stop the service $ServiceName (error: $($Error[0]))!" $LogFile
                    Exit 1
                }

                # Check if the service stopped successfully
                If (((Get-Service $ServiceName -ErrorAction SilentlyContinue).Status) -eq "Stopped" ) {
                    Acme_Logging "INFO" "The service $ServiceName was stopped successfully" $LogFile
                }
                else {
                    Acme_Logging "ERROR" "An error occurred trying to stop the service $ServiceName (error: $($Error[0]))!" $LogFile
                    Exit 1
                ...
                Acme_Logging "INFO" "The service '$ServiceName' is not running" $LogFile
            }
        }
        else {
            Acme_Logging "INFO" "The service '$ServiceName' does not exist. Nothing to do" $LogFile
...
Function Acme_StartingService {
    <#
        ...
        Depend services do not need to be specified separately. The function will retrieve them automatically.
        .EXAMPLE
        Acme_StartingService -ServiceName "Spooler"
    ...
    process {
        Acme_Logging "INFO" "Start service $ServiceName ..." $LogFile

        # Check if the service exists    
        If ( Get-Service $ServiceName -erroraction silentlycontinue) {
            # Start the main service 
            If (((Get-Service $ServiceName -ErrorAction SilentlyContinue).Status) -eq "Running" ) {
                Acme_Logging "INFO" "The service $ServiceName is already running" $LogFile
            }
            else {
                # Check for depend services and start them first
                Acme_Logging "INFO" "Check for depend services for service $ServiceName and start them" $LogFile
                $DependServices = ( ( Get-Service -Name $ServiceName -ErrorAction SilentlyContinue ).DependentServices ).name
                ...
                try {
                    ...
                    Acme_Logging "ERROR" "An error occurred trying to start the service $ServiceName (error: $($Error[0]))!" $LogFile
                    Exit 1
                }

                # Check if the service started successfully
                If (((Get-Service $ServiceName -ErrorAction SilentlyContinue).Status) -eq "Running" ) {
                    Acme_Logging "INFO" "The service $ServiceName was started successfully" $LogFile
                }
                else {
                    Acme_Logging "ERROR" "An error occurred trying to start the service $ServiceName (error: $($Error[0]))!" $LogFile
                    Exit 1
...
Function Acme_WipePrefetch {
    ...
    begin {
        ...
        Acme_Logging "INFO" "Target folder: $PrefetchFolder" $LogFile
    }

    process {
        # Check if the Prefetch folder exists
        if (!(Test-Path $PrefetchFolder)) {
            Acme_Logging "ERROR" "The Prefetch folder does not exist at the path: $PrefetchFolder" $LogFile
            Exit 1
        }
        ...
        # Delete each file
        if ($Files.Count -gt 0) {
            Acme_Logging "INFO" "Found $($Files.Count) files to delete" $LogFile
            foreach ($File in $Files) {
                try {
                    Remove-Item -Path $File.FullName -Force
                    Acme_Logging "SUCCESS" "Deleted file: $($File.FullName)" $LogFile
                }
                catch {
                    Acme_Logging "ERROR" "Failed to delete file: $($File.FullName). Error: $_" $LogFile
                }
            }
        }
        else {
            Acme_Logging "INFO" "No files found in the Prefetch folder" $LogFile
        }
    }
...
# FUNCTION Acme_StartStopProcess
#==========================================================================
Function Acme_StartStopProcess {
    <#
        .SYNOPSIS
        ...
        .EXAMPLE
        Acme_StartStopProcess -ProcessName "notepad" -Action "Start"
        Starts the notepad process.
        .EXAMPLE
        Acme_StartStopProcess -ProcessName "notepad" -Action "Stop"
    ...
    begin {
        ...
        [string]$FunctionName = $PSCmdlet.MyInvocation.MyCommand.Name
        Acme_Logging "INFO" "START FUNCTION - $FunctionName" $LogFile
        Acme_Logging "INFO" "Process Name: $ProcessName" $LogFile
        Acme_Logging "INFO" "Action: $Action" $LogFile
    }
    ...
    process {
        try {
            if ($Action -eq "Start") {
                # Start the process
                Acme_Logging "INFO" "Starting process $ProcessName" $LogFile
                Start-Process -FilePath $ProcessName -ErrorAction Stop
                Acme_Logging "SUCCESS" "Process $ProcessName started successfully." $LogFile
            }
            elseif ($Action -eq "Stop") {
                # Stop the process
                $Process = Get-Process -Name $ProcessName -ErrorAction Stop
                if ($Process) {
                    Acme_Logging "INFO" "Stopping process $ProcessName" $LogFile
                    Stop-Process -Name $ProcessName -Force -ErrorAction Stop
                    Acme_Logging "SUCCESS" "Process $ProcessName stopped successfully." $LogFile
                }
            }
        }
        catch {
            Acme_Logging "ERROR" "Error during $Action of process $ProcessName. Error: $_" $LogFile
            throw
        }
    }
...
Function Acme_ResolvePath {
    <#
        ...
        .EXAMPLE
        Acme_ResolvePath -Path "C:\Temp\MyFile.txt"
        Checks if "C:\Temp\MyFile.txt" exists and returns the resolved path if it does.
        .EXAMPLE
        Acme_ResolvePath -LiteralPath "C:\Path[With]SpecialChars"
    ...
    process {
        ...
        try {
            if ($PSCmdlet.ParameterSetName -eq "Path") {
                Acme_Logging "INFO" "Resolving path '$Path'" $LogFile
                $ResolvedPath = Resolve-Path -Path $Path -ErrorAction Stop
            }
            elseif ($PSCmdlet.ParameterSetName -eq "LiteralPath") {
                Acme_Logging "INFO" "Resolving literal path '$LiteralPath'" $LogFile
                $ResolvedPath = Resolve-Path -LiteralPath $LiteralPath -ErrorAction Stop
            }

            Acme_Logging "SUCCESS" "The resolved path is '$ResolvedPath'" $LogFile            
        }
        catch {
            Acme_Logging "WARNING" "The specified path could not be resolved." $LogFile
...
Function Acme_ChangeLocation {
    <#
        ...
        .EXAMPLE
        Acme_Set-Location -Path "C:\Temp\Projects"
    ...
    process {
        Acme_Logging "INFO" "Attempting to change to directory: '$Path'" $LogFile
        ...
        if (Test-Path -Path $Path -PathType Container) {
            try {
                ...
                Acme_Logging "SUCCESS" "Directory changed successfully to: '$newLocation'" $LogFile
                return $newLocation
            }
            catch {
                Acme_Logging "ERROR" "Failed to change directory: $($_.Exception.Message)" $LogFile
                return $null
            }
        }
        else {
            Acme_Logging "WARNING" "The directory '$Path' does not exist or is not a folder." $LogFile
            return $null
        }
    }
    end {
...
Function Acme_GetLocation {
    ...
    process {
        Acme_Logging "INFO" "Getting current working directory..." $LogFile
        
        $currentPath = Get-Location | Select-Object -ExpandProperty Path
        
        if ($currentPath) {
            Acme_Logging "SUCCESS" "Current directory: '$currentPath'" $LogFile
            return $currentPath
        }
        else {
            Acme_Logging "WARNING" "Unable to retrieve the current directory." $LogFile
            return $null
        }
    }
    end {
...
Function Acme_CheckPath {
    <#
        ...
        .EXAMPLE
        Acme_CheckPath -Path "C:\Scripts\MyFile.txt"
    ...
    process {
        Acme_Logging "INFO" "Testing existence of path: '$Path'" $LogFile
        ...
        if ($PathType) {
            ...
            Acme_Logging "INFO" "Specific path type requested: $PathType" $LogFile
        }
        
        $exists = Test-Path @params -ErrorAction SilentlyContinue
        
        if ($exists) {
            Acme_Logging "SUCCESS" "Path exists: '$Path'" $LogFile
        }
        else {
            Acme_Logging "WARNING" "Path does not exist: '$Path'" $LogFile
        }
...
Function Acme_InvokeCommand {
    <#
        ...
        .EXAMPLE
        Acme_InvokeCommand -Command "Get-Date"
    ...
    process {
        Acme_Logging "INFO" "Invoking command: '$Command'" $LogFile
        ...
        try {
            ...
            Acme_Logging "SUCCESS" "Command executed successfully." $LogFile
            return $output
        }
        catch {
            Acme_Logging "ERROR" "Failed to run command '$Command' - $($_.Exception.Message)" $LogFile
            return $null
        }
    }
...
Function Acme_GetFilename {
    <#
        ...
        .EXAMPLE
        Acme_GetFileName -File "C:\Temp\MyFile.txt"
    ...
    process {
        Acme_Logging "INFO" "Getting name from file '$File'" $LogFile
        $fileName = [System.IO.Path]::GetFileName($File)
        return $fileName

        if ($fileName) {
            Acme_Logging "SUCCESS" "The name of file: '$filename'." $LogFile
        }
        else {
            Acme_Logging "WARNING" "The file '$fileName' does not exist." $LogFile
        }
    }
...
Function Acme_MSIInstall {
    <#
        ...
        .EXAMPLE
        Acme_MSIInstall -File "C:\Path\To\Installer.msi"
        Installs the MSI package silently.
        .EXAMPLE
        Acme_MSIInstall -File "C:\Path\To\Installer.msi" -AdditionalArguments "/norestart"
    ...
    process {
        ...
        # Logging        
        Acme_Logging "INFO" "File name: $FileName" $LogFile
        Acme_Logging "INFO" "File full path: $File" $LogFile
        ...
        # Install the MSI          
        Acme_Logging "INFO" "Start the installation" $LogFile
        ...
        if ([string]::IsNullOrEmpty($AdditionalArguments)) {
            ...
            Acme_Logging "INFO" "Installin of '$File' with default arguments '$FinalArguments'" $LogFile
        }
        else {
            $FinalArguments = "$DefaultArguments $AdditionalArguments"
            Acme_Logging "INFO" "Installing '$File' with combined arguments '$FinalArguments'" $LogFile
        }
        ...
        switch ($ProcessExitCode) {        
            0 { Acme_Logging "SUCCESS" "The software was installed successfully (exit code: 0)" $LogFile }
            3 { Acme_Logging "SUCCESS" "The software was installed successfully (exit code: 3)" $LogFile }
            1603 { Acme_Logging "ERROR" "A fatal error occurred (exit code: 1603). Some applications throw this error when the software is already (correctly) installed! Please check." $LogFile }
            1605 { Acme_Logging "INFO" "The software is not currently installed on this machine (exit code: 1605)" $LogFile }
            1619 { 
                Acme_Logging "ERROR" "The installation files cannot be found. The PS1 script should be in the root directory and all source files in the subdirectory 'Files' (exit code: 1619)" $LogFile 
                Exit 1
            }
            3010 { Acme_Logging "WARNING" "A reboot is required (exit code: 3010)!" $LogFile }
            default { 
                Acme_Logging "ERROR" "The installation ended in an error (exit code: $ProcessExitCode)!" $LogFile
                Exit 1
            }
        }        
    }
...
Function Acme_SetupInstall {
    <#
        ...
        .EXAMPLE
        Acme_SetupInstall -File "C:\Temp\MyApp.exe" -Arguments "/silent /install"
    ...
    process {
        ...
        if ([string]::IsNullOrEmpty($Arguments)) {
            Acme_Logging "INFO" "Installing '$File' with no arguments" $LogFile
            $Process = Start-Process -FilePath $File -Wait -NoNewWindow -PassThru
        }
        else {
            Acme_Logging "INFO" "Installing '$File' with arguments '$Arguments'" $LogFile
        ...
        $ProcessExitCode = $Process.ExitCode
        if ($ProcessExitCode -eq 0) {
            Acme_Logging "SUCCESS" "The process '$File' completed successfully" $LogFile
        }
        else {
            Acme_Logging "ERROR" "An error occurred while executing '$File' (exit code: $ProcessExitCode)!" $LogFile
            Exit 1
        }
    }
...
Function Acme_ISOSetupInstall {
    <#
        ...
        Optional arguments to pass to the executable.
        .EXAMPLE
        Acme_ISOSetupInstall -ISO "C:\Temp\MyApp.iso" -ExeName "MyApp.exe" -ExeArgs "/silent /install"
    ...
    Acme_Logging "INFO" "Mounting $ISO." $LogFile
    Mount-DiskImage -ImagePath $ISO -ErrorAction Stop
    Acme_Logging "SUCCESS" "$ISO mounted successfully" $LogFile
    ...
    if ($driveLetter) {
        ...
        if (Test-Path -Path $exeFullPath) {
            ...
            Acme_Logging "ERROR" "The file '$exeFullPath' don't exists." $LogFile
        }
    }
    else {
        Acme_Logging "ERROR" "Unable to determine the drive letter of the ISO image." $LogFile
    }
    
    Acme_Logging "INFO" "Unmounting $ISO." $LogFile
    Dismount-DiskImage -ImagePath $ISO -ErrorAction Stop   
    Acme_Logging "SUCCESS" "Unmounting $ISO successfully." $LogFile
}
...
# FUNCTION Acme_ResolveUrl
#==========================================================================
Function Acme_ResolveUrl {
    <#
        .SYNOPSIS
        Acme_ResolveUrl resolves a URI and retrieves information such as file size, last modified date, and filename.
        .DESCRIPTION
        The Acme_ResolveUrl function resolves the given URI and retrieves information such as file size, last modified date, and filename. It sends a GET request to the URL and retrieves the headers to extract the required information.
        .PARAMETER Uri
        ...
        .EXAMPLE
        Acme_ResolveUrl -Uri 'https://example.com/file.txt'
        Resolves the URL 'https://example.com/file.txt' and retrieves the file information.
        .EXAMPLE
        Acme_ResolveUrl -Uri 'https://example.com/file.txt' -UserAgent 'My User Agent'
        Resolves the URL 'https://example.com/file.txt' using the specified user agent string.
        .EXAMPLE
        Acme_ResolveUrl -Uri 'https://example.com/file.txt' -Headers @{ 'Authorization' = 'Bearer token' }
    ...
    process {
        ...
        Acme_Logging "INFO" "Requesting headers from URL '$Uri'" $LogFile

        foreach ($UserAgentString in $UserAgent) {
            $HttpClient.DefaultRequestHeaders.Remove('User-Agent') | Out-Null
            if ($UserAgentString) {
                Acme_Logging "INFO" "Using UserAgent '$UserAgentString'" $LogFile
        ...
        if ($ResponseHeader.IsSuccessStatusCode) {
            Acme_Logging "INFO" "Successfully retrieved headers" $LogFile

            if ($ResponseHeader.RequestMessage.RequestUri.AbsoluteUri -ne $Uri) {
                Acme_Logging "INFO" "URL '$Uri' redirects to '$($ResponseHeader.RequestMessage.RequestUri.AbsoluteUri)'" $LogFile
            }
            ...
            try {
                ...
                Acme_Logging "INFO" "File size: $($FileSizeReadable)" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "Unable to determine file size" $LogFile
            }
            ...
            try {
                ...
                Acme_Logging "INFO" "Last modified: $($LastModified.ToString())" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "Last-Modified header not found" $LogFile
            }

            if ($FileName) {
                $FileName = $FileName.Trim()
                Acme_Logging "INFO" "Will use supplied filename '$FileName'"  $LogFile
            }
            else {
                ...
                try {
                    ...
                    Acme_Logging "INFO" "Content-Disposition header found: $ContentDispositionHeader" $LogFile
                }
                catch {
                    Acme_Logging "INFO" "Content-Disposition header not found" $LogFile
                }
...
'@
                    if ($ContentDispositionHeader -match $ContentDispositionRegEx) {
                        ...
                        $FileName = $FileName.Trim()
                        Acme_Logging "INFO" "Extracted filename '$FileName' from Content-Disposition header" $LogFile
                    }
                    else {
                        Acme_Logging "ERROR" "Failed to extract filename from Content-Disposition header" $LogFile
                    }
                }
                ...
                if ([string]::IsNullOrEmpty($FileName)) {
                    ...
                    [IO.Path]::GetinvalidFileNameChars() | ForEach-Object { $FileName = $FileName.Replace($_, ' ') }
                    $FileName = $FileName.Trim()
                    Acme_Logging "INFO" "Extracted filename '$FileName' from absolute URL '$($ResponseHeader.RequestMessage.RequestUri.AbsoluteUri)'" $LogFile
                }
            }
        }
        else {
        ...
        if ([string]::IsNullOrEmpty($FileName)) {
            ...
            $FileName = $FileName.Trim()
            Acme_Logging "INFO" "Extracted filename '$FileName' from original URL '$Uri'" $LogFile
        }
...
Function Acme_NovaDownloader {
    <#
        ...
        .DESCRIPTION
        The Acme_NovaDownloader function downloads a file from a specified URI and saves it to the specified destination.
        .PARAMETER Uri
        ...
        .EXAMPLE
        Acme_NovaDownloader -Uri 'https://example.com/file.txt' -Destination 'C:\Downloads'
    ...
    process {
        ...
        $Properties = Acme_ResolveUrl @ResolveUriSplat -ErrorAction Stop

        if ([string]::IsNullOrEmpty($FileName)) {
            if ([string]::IsNullOrEmpty($Properties.FileName)) {
                Acme_Logging "ERROR" "No filename found for $Uri" $LogFile
                return
            }
            else {
        ...
        if (-not (Test-Path -Path $Destination)) {
            Acme_Logging "INFO" "Output folder '$Destination' does not exist" $LogFile
            try {
                Acme_NewDirectory -Directory $Destination
                Acme_Logging "INFO" "Created output folder '$Destination'" $LogFile
            }
            catch {
                Acme_Logging "ERROR" "Unable to create output folder '$Destination': $_" $LogFile
                return
            }
        }
        ...
        try {
            ...
            Acme_Logging "ERROR" "Unable to create file '$TempFilePath': $_" $LogFile
            return
        }
                
        if ($FileStream.CanWrite) {
            Acme_Logging "INFO" "Downloading to temp file '$TempFilePath'..." $LogFile        
            ...
            while ($true) {
                try {
                    ...
                    if ($ReadBytes -eq 0) {
                        ...
                        $FileStream.Dispose()
                        try {
                            Acme_Logging "INFO" "Moving temp file to destination '$DestinationFilePath'" $LogFile
                            $DownloadedFile = Move-Item -LiteralPath $TempFilePath -Destination $DestinationFilePath -Force -PassThru
                        }
                        catch {
                            Acme_Logging "ERROR" "Error moving file from '$TempFilePath' to '$DestinationFilePath': $_" $LogFile
                            return
                        }
                       
                        if ($Properties.LastModified -and -not $IgnoreDate) {
                            Acme_Logging "INFO" "Setting Last Modified date" $LogFile
                            $DownloadedFile.LastWriteTime = $Properties.LastModified
                        }
                        Acme_Logging "INFO" "Download complete!" $LogFile
                    ...
                    Acme_Logging "ERROR" "Error downloading file: $_" $LogFile
                    Write-Progress -Activity "Downloading $FileName" -Completed
                    $FileStream.Close()
                    $FileStream.Dispose()
                    break
...
# FUNCTION Acme_CopyWithProgress
#==========================================================================
Function Acme_CopyWithProgress {
    <#
        .SYNOPSIS
        ...
        .DESCRIPTION
        Uses Acme_NewDirectory and Acme_CopyArchive internally. Displays progress, elapsed and remaining time.
    ...
    begin {
        ...
        Acme_Logging "INFO" "START FUNCTION - $FunctionName" $LogFile
        Acme_Logging "INFO" "Source: $SourcePath | Destination: $DestinationPath" $LogFile
    ...
    process {
        foreach ($item in $sourceItems) {
            ...
            if ($item.PSIsContainer) {
                Acme_Logging "INFO" "Creating directory: $destination" $LogFile
                Acme_NewDirectory -D $destination
            }
            else {
                Acme_Logging "INFO" "Copying file: $($item.FullName) -> $destination" $LogFile
                Acme_CopyArchive -F $item.FullName -D $destination
            }
        ...
        Acme_Logging "SUCCESS" "Finished copying $total item(s)." $LogFile
        Acme_Logging "INFO" "END FUNCTION - $FunctionName" $LogFile
    }
}
